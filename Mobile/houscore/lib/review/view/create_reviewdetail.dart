  ...
  void submitReview() async {
    ReviewModel reviewModel = ReviewModel(
      ...
      starRating: StarRating(
        traffic: widget.reviewData.ratings['치안']!.toDouble(),
        building: widget.reviewData.ratings['교통']!.toDouble(),
        inside: widget.reviewData.ratings['내부']!.toDouble(),
        infra: widget.reviewData.ratings['건물']!.toDouble(),
        security: widget.reviewData.ratings['인프라']!.toDouble(),
      ),
    ...
    try {
      ...
      await repository.createOneReview(image: "123123", imageName: reviewModel.address, reviewModel: reviewModel);
      print('************* CreateReview api 전송 *****************');
      Navigator.push(
          context, MaterialPageRoute(builder: (_) => CreateConfirmed()));
      print('리뷰를 작성한 주소'+reviewModel.address);
    } catch (e) {
...
class CreateOneReviewButton extends ConsumerStatefulWidget {
  const CreateOneReviewButton({super.key});

  @override
  ConsumerState<CreateOneReviewButton> createState() =>
      _RecentReviewButtonState();
}

class _RecentReviewButtonState extends ConsumerState<CreateOneReviewButton> {
  @override
  Widget build(BuildContext context) {
    final dio = Dio();
